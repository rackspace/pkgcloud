...
exports.createImage = function createImage(options, callback) {
  ...
  if (!options) {
    return errs.handle(
      errs.create({ message: 'options is a required argument.' }),
      callback
    );
  }
  else if (!options.name) {
    return errs.handle(
      errs.create({ message: 'options.name is a required argument.' }),
      callback
    );
  }
  else if (!options.server) {
    return errs.handle(
      errs.create({ message: 'options.server is a required argument.' }),
      callback
    );
  }
  ...
  if (options.metadata) {
    createImageOptions.metadata = options.metadata;
  }

  return this.serverAction(options.server, createImageOptions, function (err, body, res) {
    if (err) {
      return callback(err);
    }
    //
