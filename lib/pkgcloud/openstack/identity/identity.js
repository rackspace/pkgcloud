...
exports.createIdentity = function (options, callback) {
  ...
  if (options.client) {
    id.on('log::*', function (message, object) {
      if (object) {
        options.client.emit(this.event, message, object);
      }
      else {
        options.client.emit(this.event, message);
      }
    });
  }
...
Identity.prototype.authorize = function (options, callback) {
  ...
  request(authenticationOptions, function (err, response, body) {
    ...
    self.emit('log::debug', 'Openstack Authentication response', body);
  ...
  function getTenantId(endpoint, token) {
    ...
    self.emit('log::debug', 'Getting Tenants for OpenStack Auth', tenantOptions);

    request(tenantOptions, function (err, response, body) {

      self.emit('log::debug', 'getTenants response', body);
